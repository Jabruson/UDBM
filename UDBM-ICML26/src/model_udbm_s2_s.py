...
class UnetRes2(nn.Module):
    def __init__(
        ...
        self.unet0 = ConditionalNAFNet(img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 10], dec_blk_nums=[1, 1, 1, 1], upscale=1)
    ...
    def test(self, sample=False, last=True, FID=False,task=None):
        ...
        if self.condition:
            ...
            for items in loader:
                ...
                file_name = file_.split('/')[-1]
                save_path = str(self.results_folder /"fsf_final_small_s1" /task)
