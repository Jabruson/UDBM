...
class ConditionalNAFNet(nn.Module):
    # 48 28 or 64 16
    def __init__(self, img_channel=3, width=48, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1], upscale=1):
        super().__init__()
...
class UnetRes2(nn.Module):
    def __init__(
        ...
        self.unet0 = ConditionalNAFNet(img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 16], dec_blk_nums=[1, 1, 1, 1], upscale=1)
...
class ResidualDiffusion(nn.Module):
    ...
    def ddim_sample(self, x_input, shape, last=True, task=None):
        ...
        if self.condition:
            ...
            if not last:
                ...
                img_list = [img]
            return unnormalize_to_zero_to_one(img_list)
    def create_center_mask(self,tensor, x):
        """
        Inputs:
            tensor: input tensor with shape (n, c, h, w).
            x: retained for compatibility.
        Returns:
            result: binary mask tensor.
        """
        # Read tensor shape.
        n, c, h, w = tensor.shape
        
        # Allocate a zero mask.
        
        result = torch.zeros_like(tensor) 
        
        # Compute the masked region.
        h_inner = int(h / 2.0)
        w_inner = int(w / 2.0)
    ...
    def sample_uncertainty_aware_bridge(self, x_input, shape, model_s1=None,last=True, task=None):
        ...
        uncertain=uncertain+torch.randn_like(x_input)*1.0
    ...
    def test(self, sample=False, last=True, FID=False,task=None):
        ...
        if self.condition:
            ...
            for items in loader:
                ...
                file_name = file_.split('/')[-1]
                save_path = str(self.results_folder /"fsf_final_small_s1_ex" /task)
