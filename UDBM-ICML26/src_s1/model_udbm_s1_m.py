...
class ConditionalNAFNet(nn.Module):
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1], upscale=1):
        super().__init__()
...
class ResidualDiffusion(nn.Module):
    ...
    def p_losses(self, imgs, t, noise=None):
        ...
        if not self.condition:
            ...
            x_in = torch.cat((x_input, x_input), dim=1)
    ...
    def forward(self, img, *args, **kwargs):
        ...
        t = torch.randint(1, 2, (b,), device=device).long()
        ...
        *,
        ...
        if self.condition:
            if opts.phase == "train":
                ...
                self.dl_light = cycle(self.accelerator.prepare(DataLoader(dataset[1], batch_size=light_bs, shuffle=True, pin_memory=True, num_workers=2)))
                ...
                self.dl_blur = cycle(self.accelerator.prepare(DataLoader(dataset[4], batch_size=blur_bs, shuffle=True, pin_memory=True, num_workers=2)))
            else:
...
UncertaintyEstimator = UnetRes
StageOneDiffusion = ResidualDiffusion
StageOneTrainer = Trainer
